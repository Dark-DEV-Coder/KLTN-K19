...
QuyenTaiKhoanAdminRoute.get('/DanhSachQuyenTK', async (req, res) => {
    try {
        ...
        const quyentks = await QuyenTaiKhoan.find({ $and: [query, keywordCondition] }).limit(pageSize).skip(pageSize * page)
        const length = await QuyenTaiKhoan.find({ $and: [query, keywordCondition] }).count();
...
QuyenTaiKhoanAdminRoute.post('/Them', async (req, res) => {
    try{
        ...
        let chucnangs = [];
        let dem = 0;
        await maChucNang.forEach((element) => {
            let object = {
                MaCN: element,
        ...
        const quyentk = await QuyenTaiKhoan.create({ MaQTK: MaQTK, TenQuyenTK: TenQuyenTK, ChucNang: chucnangs });
